...
<head>
    ...
    <meta content="width=device-width, initial-scale=1.0, user-scalable=no, minimum-scale=1.0, maximum-scale=1.0"
        name="viewport" />
    <title>فاکتورهای من - دکان دارمینو</title>
...
<!-- Core CSS -->
    ...
    @include('partials.assets.datatables-styles', ['bundle' => 'full'])
            ...
            <div class="layout-page">
                ...
                <div class="content-wrapper">
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        ...
                        <!-- Sticky Actions -->
                        <form action="{{ route('invoices.myInvoices') }}" method="GET">
                            @csrf
                                    ...
                                    <div class="row mb-2">
                                        ...
                                        <div class="form-group col-6 col-md-2 mb-3">
                                            ...
                                            <input type="text" class="form-control" name="to_date" id="to_date"
                                                value="{{ isset($_GET['to_date']) ? $_GET['to_date'] : '' }}" data-jdp>
                                        </div>

                                        <div class="form-group col-6 col-md-2">
                                            <label for="delivery_from_date">نمایش از تاریخ تحویل:</label>
                                            <input type="text" class="form-control" name="delivery_from_date"
                                                id="delivery_from_date"
                                                value="{{ isset($_GET['delivery_from_date']) ? $_GET['delivery_from_date'] : '' }}"
                                                data-jdp>
                                        </div>
                                        ...
                                        <div class="form-group col-6 col-md-2">
                                            ...
                                            <input type="text" class="form-control" name="delivery_to_date"
                                                id="delivery_to_date"
                                                value="{{ isset($_GET['delivery_to_date']) ? $_GET['delivery_to_date'] : '' }}"
                                                data-jdp>
                                        </div>
                                        @if ($isLeader)
                                            <div class="form-group col-6 col-md-2 mb-3">
                                                ...
                                                <select class="select2 form-select" id="visitor_id" name="visitor_id">
                                                    ...
                                                    @foreach ($Visitors as $visitor)
                                                        <option value="{{ $visitor->id }}"
                                                            @if (isset($_GET['visitor_id']) && $visitor->id == $_GET['visitor_id']) selected @endif>
                                                            {{ $visitor->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <select class="select2 form-select" data-allow-clear="true" multiple
                                                ...
                                                @foreach ($Cities as $city)
                                                    <option value="{{ $city->id }}"
                                                        @if (isset($_GET['city_id']) && $city->id == $_GET['city_id']) selected @endif>
                                                        {{ $city->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                            ...
                                            @if (isset($PishFactors))
                                                @foreach ($PishFactors as $invoice)
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td width="7%"><small data-bs-toggle="tooltip"
                                                        ...
                                                        <td width="12%">
                                                            <a href="{{ route('pishFactorInfo', $invoice->id) }}">
                                                                {{ isset($invoice->customer->name) ? $invoice->customer->name : ($invoice->visitor ? $invoice->visitor->name : 'وارد نشده') }}
                                                            </a>
                                                        </td>
                                                        <td width="12%">
                                                            @if ($invoice->is_agency_order && $invoice->visitor)
                                                                <small class="badge bg-label-info">برای خودش</small>
                                                                <br>
                                                                <small>{{ $invoice->agencyUser ? $invoice->agencyUser->name : $invoice->visitor->name }}</small>
                                                            @else
                                                                <small>مشتری</small>
                                                            @endif
                                                        </td>
                                                        <td class="text-center" width="80">
                                                            ...
                                                            <small data-bs-toggle="tooltip" data-bs-placement="top"
                                                                ...
                                                                @if ($Packs > 0)
                                                                    {{ $Packs }}
                                                                    {{ $organ ? $organ->sub_unit : 'واحد پخش حذف شده' }}
                                                                @endif
                                                                @if ($tedad > 0)
                                                                    {{ $tedad }}
                                                                    {{ $organ ? $organ->unit_order : 'واحد پخش حذف شده' }}
                                                                @endif
