...
<head>
    ...
    <title>عملیات و مسیرهای من - دکان دارمینو</title>
                                ...
                                <div class="card-datatable table-responsive py-0">
                                    <table class="datatables-direct-basic table">
                                        ...
                                        <tr class="text-center">
                                            <th width="40" style="max-width: 40px">ردیف</th>
                                            <th>محل عملیات</th>
                                            <th>جزئیات فروش</th>
                                        </tr>
                                        </thead>
                                        ...
                                        @foreach($MyTasks as $task)
                                                <?php $task_area = DB::table('areas')->where('id', $task->area_id)->first() ?>
                                                <?php $task_region = DB::table('regions')->where('id', $task_area->region_id)->first() ?>
                                            <tr>
                                                <th width="40" style="max-width: 40px">{{ $x }}</th>
                                                <td><a href="{{ route('tasks.visitor_list', $task->id) }}"><small>{{ $task_region->name }} - {{ $task_area->name }}</small></a> </td>
                                                <td class="text-center">
                                                        <?php $task_factors = DB::table('pishfactors')->where('task_id', $task->id)->get() ?>
                                                        <?php $task_accepted_factors = DB::table('pishfactors')->whereIn('status',[1,4])->where('task_id', $task->id)->get() ?>
                                                        <?php
                                                        $total_full_price = DB::table('pishfactors')
                                                            ->whereIn('status', [1, 4])
                                                            ->where('task_id', $task->id)
                                                            ->sum('fullPrice'); ?>
                                                    <span class="badge bg-label-primary me-1">{{ count($task_factors) }} فاکتور ثبت شده</span> <br />
                                                    <span class="badge bg-label-success me-1">{{ count($task_accepted_factors) }} فاکتور تایید شده</span> <br />
                                                    <small class="badge bg-label-success me-1">{{ number_format($total_full_price) }} ریال مجموع فروش</small>
                                                </td>
                                            </tr>
                                            @php($x++)
                                        @endforeach
