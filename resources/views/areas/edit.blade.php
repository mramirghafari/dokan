...
<head>
    ...
    <meta content="width=device-width, initial-scale=1.0, user-scalable=no, minimum-scale=1.0, maximum-scale=1.0"
        name="viewport" />
    <title>ویرایش مسیرهای سامانه - دکان دارمینو</title>
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        ...
                        <div class="row mt-5">
                            <div class="col-12 order-1 order-lg-2 mb-4 mb-lg-0">
                                <div class="card mb-4">
                                    <div class="card-body">
                                        <form action="{{ route('areas.update', $area->id) }}" class="row"
                                            method="POST">
                                            @csrf
                                            @method('PATCH')
                                                ...
                                                <select class="select2 form-select" data-allow-clear="true"
                                                    ...
                                                    @foreach ($Regions as $region)
                                                        <option value="{{ $region->id }}"
                                                            @if ($area->region_id == $region->id) selected @endif>
                                                            {{ $region->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                                ...
                                                <select class="select2 form-select" data-allow-clear="true"
                                                    ...
                                                    @foreach ($Leaders as $leader)
                                                        <option value="{{ $leader->id }}"
                                                            @if ($area->leader_id == $leader->id) selected @endif>
                                                            {{ $leader->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="mb-3 col-12 col-md-4">
                                                ...
                                                <select class="form-select" id="visit_frequency" name="visit_frequency">
                                                    @foreach (\App\Models\Area::VISIT_FREQUENCY_LABELS as $value => $label)
                                                        <option value="{{ $value }}"
                                                            @if (($area->visit_frequency ?: 'weekly') === $value) selected @endif>
                                                            {{ $label }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="mb-3 col-12 col-md-8">
                                                ...
                                                @php($selectedVisitDays = $area->visit_days ?: [])
                                                <div class="d-flex flex-wrap gap-3">
                                                    @foreach (\App\Models\Area::VISIT_DAY_LABELS as $value => $label)
                                                        <label class="form-check mb-0">
                                                            <input class="form-check-input" name="visit_days[]"
                                                                type="checkbox" value="{{ $value }}"
                                                                @if (in_array($value, $selectedVisitDays)) checked @endif>
                                                            <span class="form-check-label">{{ $label }}</span>
                                                        </label>
                                                    @endforeach
                                                </div>
                                            </div>
                                            <button class="btn btn-primary" type="submit">به روزرسانی مسیر</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
                                            ...
                                            <tbody>
                                                ...
                                                @foreach ($Areas as $area)
                                                    <tr>
                                                        ...
                                                        <td>{{ $area->visitDaysText() }}</td>
                                                        <td>{{ $area->visitFrequencyText() }}</td>
                                                        <td>
