...
<head>
    ...
    <title>ویرایش شهرهای سامانه - دکان دارمینو</title>
                ...
                <div class="container-xxl flex-grow-1 container-p-y">
                    ...
                    <div class="row mt-5">
                        <div class="col-12 order-1 order-lg-2 mb-4 mb-lg-0">
                            <div class="card mb-4">
                                <div class="card-body">
                                    <form action="{{ route('cities.update', $city->id) }}" class="row" method="POST">
                                        @csrf
                                        @method('PATCH')
                                        <div class="mb-3 col-12 col-md-4">
                                            <label class="form-label" for="name">نام شهر</label>
                                            <input class="form-control" id="name" placeholder="نام شهر" name="name" value="{{ $city->name }}" required type="text"/>
                                        </div>
                                        <div class="mb-3 col-12 col-md-4">
                                            ...
                                            <select class="select2 form-select" data-allow-clear="true" id="organization_id" name="organization_id">
                                                ...
                                                @foreach ($organizations as $organ)
                                                    <option value="{{ $organ->id }}" @if($city->organization_id == $organ->id) selected @endif>{{ $organ->title }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <button class="btn btn-primary" type="submit">به روزرسانی شهر</button>
                                    </form>
                                </div>
                            </div>
                        </div>
