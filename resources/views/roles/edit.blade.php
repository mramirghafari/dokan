...
<head>
    ...
    <meta content="width=device-width, initial-scale=1.0, user-scalable=no, minimum-scale=1.0, maximum-scale=1.0"
        name="viewport" />
    <title>ویرایش نقش کاربری - دکان دارمینو</title>
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        ...
                        <div class="row mt-5">
                            <div class="col-12 order-1 order-lg-2 mb-4 mb-lg-0">
                                <div class="card mb-4">
                                    <div class="card-body">
                                        <form action="{{ route('roles.update', $role->id) }}" class="row"
                                            method="POST">
                                            @csrf
                                            @method('PATCH')
                                                    ...
                                                    <select class="select2 form-select" id="tenant_id" name="tenant_id"
                                                        ...
                                                        @foreach ($tenants as $tenant)
                                                            <option value="{{ $tenant->id }}"
                                                                @if ((int) ($role->tenant_id ?: $targetTenantId) === (int) $tenant->id) selected @endif>
                                                                {{ $tenant->name }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            ...
                                            @endif
                                            <div class="mb-3 col-12 col-md-4">
                                                <label class="form-label" for="max_discount_percent">حداکثر تخفیف (درصد)</label>
                                                <input class="form-control" id="max_discount_percent" name="max_discount_percent"
                                                    type="number" min="0" max="100" step="0.01"
                                                    value="{{ old('max_discount_percent', $role->max_discount_percent) }}" />
                                                <small class="text-muted">سقف درصد تخفیف برای کاربران این نقش در ثبت سفارش.</small>
                                            </div>
                                            <div class="mb-3 col-12 col-md-4">
                                                <label class="form-label" for="max_discount_amount">حداکثر مبلغ تخفیف (ریال)</label>
                                                <input class="form-control" id="max_discount_amount" name="max_discount_amount"
                                                    type="number" min="0" step="1"
                                                    value="{{ old('max_discount_amount', $role->max_discount_amount) }}" />
                                            </div>
                                            ...
                                            <button class="btn btn-primary" type="submit">ویرایش نقش کاربری</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
