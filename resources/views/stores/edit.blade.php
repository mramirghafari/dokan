                                        ...
                                        <form action="{{ route('stores.update', $store->id) }}" method="POST">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input class="form-control" id="description" placeholder="توضیح"
                                                    type="text" name="description" value="{{ $store->description }}"
                                                    required />
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label" for="code">کد انبار</label>
                                                <input class="form-control" id="code" placeholder="کد انبار"
                                                    type="number" name="code" value="{{ $store->code }}"
                                                    required />
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label" for="store_type">نوع انبار</label>
                                                <select class="form-select" id="store_type" name="store_type" required>
                                                    @foreach ($storeTypes as $value => $label)
                                                        <option value="{{ $value }}"
                                                            @if (old('store_type', $store->store_type ?: 'main') === $value) selected @endif>
                                                            {{ $label }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="mb-3">
                                                ...
                                                <select class="form-select" id="stock_tracking_mode"
                                                    ...
                                                    @foreach ($stockTrackingModes as $value => $label)
                                                        <option value="{{ $value }}"
                                                            @if (old('stock_tracking_mode', $store->stock_tracking_mode ?: 'tracked') === $value) selected @endif>
                                                            {{ $label }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label" for="transfer_policy">سیاست انتقال</label>
                                                <select class="form-select" id="transfer_policy" name="transfer_policy"
                                                    required>
                                                    @foreach ($transferPolicies as $value => $label)
                                                        <option value="{{ $value }}"
                                                            @if (old('transfer_policy', $store->transfer_policy ?: 'in_out') === $value) selected @endif>
                                                            {{ $label }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="mb-3">
                                                ...
                                                <select class="select2 form-select" name="organization_id[]" multiple
                                                    style="width: 100%;">
                                                    <option value="">--هیچکدام--</option>
                                                    @foreach ($organizations as $organization)
                                                        <option value="{{ $organization->id }}"
                                                            @if (in_array($organization->id, old('organization_id', $selectedOrganizationIds))) selected @endif>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select class="select2 form-select" id="pr_ids" name="pr_ids[]"
                                                    multiple style="width: 100%;">
                                                    <option value="">--هیچکدام--</option>
                                                    @foreach ($Products as $pr)
                                                        <option value="{{ $pr->id }}"
                                                            @if (is_array($store->pr_ids) && in_array($pr->id, $store->pr_ids)) selected @endif>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input type="text" class="form-control" name="lat"
                                                    placeholder="lat..." value="{{ $store->lat }}" />
                                            </div>
                                            <div class="mb-3">
                                                <label for="pr_ids">موقعیت مکانی انبار lang:</label>
                                                <input type="text" class="form-control" name="lang"
                                                    placeholder="lang..." value="{{ $store->lang }}" />
                                            </div>
                                            <div class="mb-3">
                                                ...
                                                <select class="select2 form-select" data-allow-clear="true"
                                                    ...
                                                    @foreach ($roles as $role)
                                                        <option value="{{ $role->id }}"
                                                            {{ in_array($role->id, $store->roles->pluck('id')->toArray()) ? 'selected' : '' }}>
                                                            {{ $role->title }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
