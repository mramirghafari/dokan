...
<head>
    ...
    <title>عملیات ورود دیتا - دکان دارمینو</title>
                ...
                <div class="container-xxl flex-grow-1 container-p-y">
                    <div class="d-flex justify-content-between align-items-center gap-3 flex-wrap mb-4" id="tour-import-page-header">
                        <div id="tour-import-page-title">
                            ...
                            <div class="text-muted">ورود گروهی مشتری، واریزی و فاکتور از فایل CSV — کلید یکتا مشتری: موبایل در همین پنل.</div>
                        </div>
                        <div>
                            <a class="btn btn-label-secondary" id="tour-import-template" href="{{ route('customers.data-import.template') }}">
                    ...
                    <div class="row g-4">
                        <div class="col-lg-5">
                            <div class="card h-100" id="tour-import-upload-card">
                                ...
                                <div class="card-body">
                                    <form id="customer-import-form" method="POST" action="{{ route('customers.data-import.import') }}" enctype="multipart/form-data">
                                        @csrf
                                        ...
                                        @if($users->isNotEmpty())
                                            <div class="mb-3" id="tour-import-registrar">
                                                <label class="form-label" for="default_registrar_user_id">ثبت‌کننده پیش‌فرض (اختیاری)</label>
                                                <select name="default_registrar_user_id" id="default_registrar_user_id" class="form-select select2">
                                                    <option value="">— بدون پیش‌فرض —</option>
                                                    @foreach($users as $registrar)
                                                        <option value="{{ $registrar->id }}" @selected(old('default_registrar_user_id') == $registrar->id)>{{ $registrar->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        @endif
                                        ...
                                        <div class="form-check mb-2" id="tour-import-update-existing">
                                            ...
                                            <label class="form-check-label fw-semibold" for="update_existing">ثبت واریزی‌های جدید برای مشتری موجود</label>
                                        </div>
                                        <p class="small text-muted mb-4">
                                            اگر موبایل در سیستم وجود دارد، با فعال بودن این گزینه <strong>واریزی و خرید جدید</strong> ثبت می‌شود؛
                                            مشتری دوباره ساخته نمی‌شود. برای بار اول import معمولاً لازم نیست.
                                        </p>
                                        ...
                                        <div id="tour-import-progress" class="import-progress-panel d-none mt-4" aria-live="polite">
                                            ...
                                            <div class="d-flex flex-wrap gap-2 mt-3" id="import-progress-steps">
                                                ...
                                                <span class="badge bg-label-secondary import-step" data-step="importing">۴. ثبت مشتریان</span>
                                ...
                                <div class="card-body">
                                    <div class="alert alert-info py-2 px-3 mb-3 small">
                                        ...
                                        <ul class="mb-0 mt-2 pe-3">
                                            <li>فایل باید سرستون فارسی مطابق <strong>قالب CSV</strong> داشته باشد — همه ۱۴ ستون در قالب وجود دارد.</li>
                                            <li>ستون‌های <strong>نام</strong> و <strong>موبایل</strong> در هر ردیف <strong>الزامی</strong> هستند.</li>
                                            <li>بقیه ستون‌ها اختیاری‌اند و می‌توانند خالی بمانند.</li>
                                            <li>ردیف‌هایی که <strong>هم نام و هم موبایل خالی</strong> باشند نادیده گرفته می‌شوند (وارد نمی‌شوند).</li>
                                            <li>اگر فقط یکی از «نام» یا «موبایل» پر باشد، ردیف <strong>رد می‌شود</strong> و در گزارش خطا نمایش داده می‌شود.</li>
                                        </ul>
                                    </div>

                                    <h6 class="fw-semibold mb-2">ستون‌های فایل</h6>
                                    ...
                                    <h6 class="fw-semibold mb-2">شناسایی مشتری</h6>
                                    <ul class="mb-3 ps-3 small">
                                        <li class="mb-1"><strong>موبایل</strong> کلید یکتا مشتری در همین پنل است؛ یک نفر = یک پرونده مشتری.</li>
                                        <li class="mb-1">نام، کد ملی و تابلو در ردیف‌های بعدی می‌توانند تکرار شوند (همان مشتری).</li>
                                    </ul>

                                    <h6 class="fw-semibold mb-2">منظور از «ثبت واریزی‌های جدید» چیست؟</h6>
                                    <p class="small text-muted mb-2">
                                        یک مشتری می‌تواند در چند ردیف اکسل بیاید. هر ردیف یک <strong>تراکنش مالی</strong> است، نه مشتری جدید:
                                    </p>
                                    <ul class="mb-3 ps-3 small">
                                        <li class="mb-1"><strong>پیش‌پرداخت</strong> — واریزی جزئی ثبت می‌شود (مثلاً ۱۰٬۰۰۰٬۰۰۰ و ۸۰۰٬۰۰۰ {{ currency_label() }}).</li>
                                        <li class="mb-1"><strong>تکمیل وجه</strong> — خرید همان دوره تکمیل و <strong>فاکتور کامل</strong> صادر می‌شود.</li>
                                        <li class="mb-1">اگر مشتری <strong>۳ بار «تکمیل وجه»</strong> داشته باشد یعنی <strong>۳ خرید جدا</strong> — برای هر کدام یک فاکتور مستقل.</li>
                                    </ul>

                                    <div class="alert alert-secondary py-2 px-3 mb-3 small">
                                        <strong>مثال — آقای علی رضایی (موبایل ۰۹۱۲***۳۴۵۶):</strong>
                                        <ol class="mb-0 mt-2 pe-3">
                                            <li>ردیف ۱: پیش‌پرداخت ۱٬۰۰۰٬۰۰۰ — واریزی ثبت</li>
                                            <li>ردیف ۲: پیش‌پرداخت ۸۰۰٬۰۰۰ — واریزی ثبت</li>
                                            <li>ردیف ۳: تکمیل وجه ۶۰۰٬۰۰۰ (اشتراک ۲ ماهه) — فاکتور کامل همان خرید</li>
                                        </ol>
                                    </div>

                                    <h6 class="fw-semibold mb-2">در صورت به‌روزرسانی فایل اکسل و افزودن ردیف‌های بیشتر</h6>
                                    <ul class="mb-3 ps-3 small">
                                        <li class="mb-1">ردیف‌هایی که قبلاً پردازش شده‌اند <strong>دوباره ایجاد نمی‌شوند</strong> — فقط ردیف‌های جدید اضافه‌شده پردازش می‌شوند.</li>
                                        <li class="mb-1">برای افزودن واریزی یا خرید جدید به مشتری موجود، گزینه «ثبت واریزی‌های جدید» را فعال کنید.</li>
...
<script>
    $(function () {
        ...
        const statusUrl = @json(route('customers.data-import.status', ['run' => '__RUN__']));
        const importUrl = @json(route('customers.data-import.import'));
        ...
        const $form = $('#customer-import-form');
