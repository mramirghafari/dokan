...
class MobileController extends Controller
{
    public function login(Request $request): JsonResponse
    {
        ...
        if (!$user || !Hash::check($data['password'], $user->password)) {
            ActivityLogService::logFailedLogin($data['username'] ?? $data['mobile'] ?? null, $user, 'اعتبارسنجی ناموفق');
            throw ValidationException::withMessages(['username' => 'Invalid login credentials.']);
        }

        if ($message = $user->loginBlockMessage()) {
            ActivityLogService::logFailedLogin($data['username'] ?? $data['mobile'] ?? null, $user, 'حساب مسدود');
