...
class RoleController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        ActivityLogService::safeLogModel('create', 'نقش جدید ایجاد شد' . '-' . $role->description, $role, ['section' => 'roles', 'event_key' => 'role.create']);
    ...
    public function update(Request $request, Role $role)
    {
        ...
        $roles = $scopeService->rolesForUser(auth()->user())->with('scopes')->orderBy('description')->get();
        $permissions = $scopeService->permissionsForUser(auth()->user())->orderBy('description')->get();
        ActivityLogService::safeLogModel('update', 'نقش ویرایش شد' . '-' . $role->description, $role, ['section' => 'roles', 'event_key' => 'role.update']);
