...
class ActivityLogService
{
    ...
    public static function safeLog(
        ...
        try {
            return self::writeLog($action, $description, $userId, $context);
    ...
    {
        try {
            return self::writeLogModel($action, $description, $source, $context);
        } catch (Throwable $exception) {
            Logger::warning('Activity log failed', [
                'action' => $action,
                'description' => $description,
    ...
    private static function writeLogModel(string $action, string $description, ?Model $source = null, array $context = []): ?Log
    {
        if ($source) {
            $context['section'] = $context['section'] ?? class_basename($source);
            $context['section_id'] = $context['section_id'] ?? $source->getKey() ?? 0;
        ...
        return self::writeLog($action, $description, null, $context);
    }
    ...
    private static function writeLog(
        ...
        $attributes = [
            ...
            'section_id' => self::normalizeSectionId($context['section_id'] ?? 0),
    ...
    private static function normalizeSectionId(mixed $sectionId): int
    {
        if ($sectionId === null || $sectionId === '') {
            return 0;
        }

        return max(0, (int) $sectionId);
    }
}
